...
function setup(){
 ...
 function syncFiles(){try{const dt=new DataTransfer();queue.forEach(f=>dt.items.add(f));const images=document.getElementById('lessonImages');if(images)images.files=dt.files;window.lessonImageFiles=queue.filter(f=>f.type&&f.type.startsWith('image/'));}catch(err){console.warn('CanvasFlow attachments:',err);}}
 function run(mode='explain'){if(busy)return;busy=true;syncFiles();const text=input.value.trim();if(text){const messages=chat.querySelector('.cf-mobile-messages');if(messages){const b=document.createElement('div');b.className='cf-mobile-bubble user';b.textContent=text;messages.appendChild(b);messages.scrollTop=messages.scrollHeight;}}input.value='';queue=[];render();if(typeof window.runAI==='function'){Promise.resolve(window.runAI(mode)).finally(()=>{busy=false;});}else{busy=false;}}
...
console.log('CanvasFlow: normal mobile AI chat behavior installed.');
