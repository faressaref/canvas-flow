...
const js = `<script id="canvasflow-desktop-ai-chat-fix">(function(){
  // This is the tablet/desktop chat. Mobile has its own composer.
  ...
  function setup(){
    ...
    // Keep the original action buttons in the panel, but place them below chat.
    body.innerHTML='';
    body.appendChild(chat);
    body.appendChild(actions);
    const previews=document.getElementById('lessonPreviews');
    ...
    // One hidden universal file picker. Images are forwarded to the existing
    // lesson image pipeline; PDFs/docs/text are attached and text files are read.
    let picker=document.getElementById('canvasflowDesktopFileInput');
    if(!picker){
      picker=document.createElement('input');
      picker.id='canvasflowDesktopFileInput';
      picker.type='file';
      picker.accept='image/*,.pdf,.txt,.doc,.docx,.ppt,.pptx';
      picker.style.display='none';
      document.body.appendChild(picker);
    }
    composer.querySelector('.cf-desktop-attach').onclick=e=>{
    ...
    picker.onchange=()=>{
      ...
      if(file.type&&file.type.startsWith('image/')&&images){
        ...
        let chip=document.getElementById('canvasflowDesktopAttachment');
        if(!chip){
          chip=document.createElement('div');
          chip.id='canvasflowDesktopAttachment';
          chip.className='cf-desktop-attachment';
        ...
        if(file.type==='text/plain'){
          ...
          reader.onload=()=>{input.value=String(reader.result||'').slice(0,12000);};
    ...
    const send=()=>{
      ...
      if(!value && !(window.lessonImageFiles||[]).length && !document.getElementById('canvasflowDesktopAttachment')){
        bubble('اكتب سؤالك أو ارفع صورة/ملف الأول 📎','ai');
        return;
      }
    ...
    // Mirror AI output into chat after the existing AI function finishes.
    const originalRunAI=window.runAI;
    if(typeof originalRunAI==='function'&&!originalRunAI.__canvasflowChatWrapped){
      const wrapped=async function(mode){
        await originalRunAI(mode);
        const text=(result.textContent||'').trim();
        if(text && !/^جاري/.test(text)){
          bubble(text,'ai');
          result.textContent='';
        }
      };
      wrapped.__canvasflowChatWrapped=true;
  ...
  setup();
  [250,600,1200,2500].forEach(t=>setTimeout(setup,t));
  const observer=new MutationObserver(()=>{if(!installed||!document.getElementById('canvasflowDesktopChat')) setup();});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('resize',()=>{if(!isWide())installed=false;setTimeout(setup,100);},{passive:true});
})();</script>`;
...
@media screen and (min-width:761px){
  ...
  .cf-desktop-welcome{align-self:flex-start;max-width:88%;padding:10px 12px;border:1px solid #e1e5eb;border-radius:13px 13px 13px 5px;background:#fff;color:#39424e;font-size:11px;line-height:1.65;}
  .cf-desktop-bubble{max-width:88%;padding:9px 11px;border-radius:13px;font-size:11px;line-height:1.7;white-space:pre-wrap;word-break:break-word;}
  .cf-desktop-bubble.user{align-self:flex-end;background:#18202a;color:#fff;border-radius:13px 13px 5px 13px;}
  .cf-desktop-bubble.ai{align-self:flex-start;background:#fff;color:#303844;border:1px solid #e1e5eb;border-radius:13px 13px 13px 5px;}
  ...
  .cf-desktop-attach{background:#eef1f5;color:#39424e;}
  .cf-desktop-send{background:#18202a;color:#fff;}
  ...
  #aiPanel .ai-action b{display:inline;font-size:9px;margin:0;}
  #aiPanel .ai-action span{display:none;}
  #aiPanel #lessonPreviews{flex:0 0 auto;max-height:54px;overflow:auto;margin:0;display:flex;gap:5px;}
  #aiPanel #lessonPreviews:empty{display:none;}
  #aiPanel #aiResult{display:none!important;}
  #aiPanel #aiStatus{flex:0 0 auto;font-size:8px;margin:0;min-height:10px;}
  ...
  @media (max-width:1000px){
    #aiPanel{width:min(92vw,520px)!important;max-width:92vw!important;height:min(76dvh,620px)!important;}
  }
}
</style>`;
...
console.log('CanvasFlow: reliable AI chat restored for tablet and desktop.');
